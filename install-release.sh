...
TMP_DIRECTORY="$(mktemp -dt xray.XXXXXXXXXX)"
ZIP_FILE="${TMP_DIRECTORY}/Xray-linux-$MACHINE.zip"
LATEST_RELEASE=$(curl -s https://github.com/XTLS/Xray-core/releases/latest | grep -oE "\/tag\/v([[:digit:]]|\.)+" | grep -oE "v([[:digit:]]|\.)+")
DOWNLOAD_LINK="https://github.com/XTLS/Xray-core/releases/download/${LATEST_RELEASE}/Xray-linux-$MACHINE.zip"
...
install_startup_service_file() {
    ...
    if [ ! -f '/etc/init.d/xray' ]; then
        ...
        curl -o "${TMP_DIRECTORY}/init.d/xray" https://raw.githubusercontent.workers.dev/Zerorigin/alpinelinux-install-xray/master/init.d/xray -s
        if [ "$?" -ne '0' ]; then
